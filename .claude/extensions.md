...
| `ext-mirror` | **Mirror drawing mode** + mirror-copy action. View-tray toggle `tool_mirror` (click = vertical canvas-center axis, Shift+click = horizontal) with a dashed axis overlay (`#mirrorAxis` on `#canvasBackground`). Twin creation hooks history, not events: wraps `svgCanvas.addCommandToHistory` and replaces a bare `InsertElementCommand` of a layer child (= hand-drawn commit) with one `BatchCommand` of source + reflected twin (`se:mirror-of`), so a mirrored stroke undoes in one step; batched inserts (paste/repeat/bool-ops) pass through. Text twins are position-mirrored only. `svgCanvas.mirrorSelection()` (buttons `tool_mirror_copy`/`_multi`) reflects a copy of the selection without the mode | `ext-mirror.js` |
