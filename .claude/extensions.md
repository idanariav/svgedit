...
| `ext-shadow` | Drop shadow on any single selected element — angle/length, blur, opacity, color; uses an `<feDropShadow>` filter in `<defs>`. **Length is the on/off control: length 0 ⟺ no shadow** (the panel defaults to 0 when the element has none, and setting length 0 removes the filter). The filter region uses **`objectBoundingBox`** units (fractions of the element bbox) so it tracks the element when moved/duplicated/pasted — an absolute `userSpaceOnUse` region is left behind on a move and hides the element. Legacy absolute regions are normalized on load by `svgCanvas.convertDropShadowFilters` (svg-exec.js). Exposes `svgEditor.shadowApi` (`{ read(elem), apply(elem, params, batchCmd) }`) so the class library can capture/re-stamp shadows | `ext-shadow.js` |
