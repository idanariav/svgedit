...
| `ext-shadow` | Drop shadow on any single selected element — offset X/Y, blur, opacity, color; uses an `<feDropShadow>` filter in `<defs>`. The filter region uses **`objectBoundingBox`** units (fractions of the element bbox) so it tracks the element when moved/duplicated/pasted — an absolute `userSpaceOnUse` region is left behind on a move and hides the element. Legacy absolute regions are normalized on load by `svgCanvas.convertDropShadowFilters` (svg-exec.js) | `ext-shadow.js` |
