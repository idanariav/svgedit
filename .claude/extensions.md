...
| `ext-layer_view` | **Layer Focus** mode (`#tool_layerView`, `Ctrl+Shift+L`). Toggling it isolates the current layer: every other layer is **locked** (`setLayerLocked` — new/pasted objects can only land on the focused layer) and **dimmed** (inline `style.opacity` on the layer `<g>`, so it survives the panel's attribute-based hover highlight), the focused layer's name shows in an on-canvas badge (`#layer_focus_badge`, a grid-cell sibling of `#workarea`), and layer-nav hotkeys activate while on: `[`/`]` switch the focused layer down/up, `PageUp`/`PageDown` move the selection to the adjacent layer and follow it. Snapshots each layer's lock state on entry, restores it on exit; nothing is persisted. Re-derives on `layersChanged`; drops out on manual `layerVisChanged`. Honours the `curConfig.layerView` startup flag | `ext-layer_view.js` |
...
| `ext-repeat` | **Radial / grid / path repeat** (array) tool. `svgCanvas.repeatSelection(params)` clones the selection per step (radial: `rotate(a cx cy)` prepended, never recalculated; grid: `translate` steps of stroked-bbox+gap; **path**: copies distributed along a rail `<path>` by arc length via `getPointAtLength` — count/start%/span%, optional tangent **rotation-follow** — the rail is the topmost selected path on a fresh apply, or the `rail=<id>` stored in the params stamp on re-edit, and is never cloned/stamped; closed rails wrap i/n, open rails clamp endpoints) with fresh ids (`remapElementIdsAndRefs`; defs stay shared), one `BatchCommand`. Re-editable via stamps: `se:repeat-source`/`se:repeat` (params) on sources, `se:repeat-copy` on copies — `getRepeatParams()` seeds the `<se-repeat-settings>` popover from source *or copy*, and re-applying replaces the copies in the same batch. Buttons injected into the Object + Combine sections | `ext-repeat.js`, `components/seRepeatSettings.js` |
| `ext-mirror` | **Mirror drawing mode** + mirror-copy action + **live linked symmetry**. View-tray toggle `tool_mirror` (click = vertical canvas-center axis, Shift+click = horizontal) with a dashed axis overlay (`#mirrorAxis` on `#canvasBackground`). Twin creation hooks history, not events: wraps `svgCanvas.addCommandToHistory` and replaces a bare `InsertElementCommand` of a layer child (= hand-drawn commit) with one `BatchCommand` of source + reflected twin (`se:mirror-of` + `se:mirror-axis`), so a mirrored stroke undoes in one step; batched inserts (paste/repeat/bool-ops) pass through. Text twins are position-mirrored only. **Live link:** `elementChanged`/`elementTransition` re-sync a twin whenever its source changes (during the drag too), mode on or off; syncs are non-undoable (connector re-routing precedent — undo of the source edit re-fires the sync). Dragging a twin directly (or source+twin together) unlinks it so manual edits stick. **Mirror delete:** wraps `svgCanvas.deleteSelectedElements` to pull a still-linked partner (source or twin) into the selection before the original runs, so deleting either half removes both in one undo step; unlinked elements delete normally. `svgCanvas.mirrorSelection()` (buttons `tool_mirror_copy`/`_multi`) reflects a copy of the selection without the mode | `ext-mirror.js` |
| `ext-guides` | **Draggable ruler guides + composition overlays.** Guides live in `#rulerGuides`, an *interactive* overlay inserted before `#smartGuides` in `svgroot` (above content): drag one to reposition (invisible 9px hit lines, `pointer-events: stroke`), drag it off the canvas to delete. Created by dragging off a ruler (when rulers are enabled — this fork defaults `showRulers: false`) or via the popover's "+ Vertical/Horizontal" buttons (`svgCanvas.addRulerGuide(axis)`, drops at canvas center). Positions persist as `se:guides="v:…;h:…"` on `#svgcontent` (sanitize `se:` bypass; re-read on document load via the `elementChanged`-with-`<svg>` signal). While dragging objects, guides are single-axis snap targets injected in `core/smart-guides.js` `collectSnapTargets` via `svgCanvas.getRulerGuides()` (`isGuide` targets; matched guides highlight full-length). Composition overlays (rule-of-thirds / golden ratio / center cross) draw in `#compOverlays` on `#canvasBackground`, toggled from the `<se-guides-settings>` view-tray popover, persisted as `guides_thirds/golden/center` prefs | `ext-guides.js`, `components/seGuidesSettings.js` |
| `ext-motion-lines` | **Parametric motion-line (speed-line) generator** — `svgCanvas.applyMotionLines({dir,count,len,gap,curve})` builds a `<g>` of trailing quadratic paths beside the selection's stroked bbox (middle line longest, outer lines bowed apart by `curve`; paint/width follow the source's stroke → fill → current style, round caps). Re-editable via the ext-repeat stamp scheme: `se:motion-source`/`se:motion` (params) on sources, `se:motion-copy` on the group; `getMotionParams()` seeds the `<se-motion-settings>` popover and Apply→Update regenerates in one batch. Buttons in the Object + Combine sections | `ext-motion-lines.js`, `components/seMotionSettings.js` |
...
| `ext-curvature` | Curvature tool — click-to-place smooth curves via Spiro (clothoid curves, `spiro` pkg); Shift+click for corner anchors; double-click/Escape or click-start to finalize | `ext-curvature.js` |
| `ext-brush` | Pressure-sensitive freehand **Brush** tool (mode `'brush'`, `#tool_brush`). Renders a *filled* `<path>` outline via the `perfect-freehand` pkg — variable width driven by pen pressure (Apple Pencil/Wacom/Surface). The drawing pipeline is mouse-based, so coordinates flow through the normal `mouseDown/Move/Up` hooks while **pen pressure rides a passive side-channel**: a module-scope `pointerdown`/`pointermove` listener on `svgroot` records the latest `pressure`+`pointerType` (pointer events fire just before their compat mouse events). Real pressure only for `pointerType==='pen'`; mouse/finger get `simulatePressure` (velocity taper). `size` derives from the stroke-width control; fill = current stroke colour. History/selection are committed by core on the `{keep,element}` return — the extension does **not** call `addCommandToHistory` | `ext-brush.js` |
