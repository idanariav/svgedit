...
| `seSettingsPopover.js` | *(base class, not a custom element)* | `SeSettingsPopover` — shared base for the six toolbar-button-plus-popover settings components below: shadow-DOM/icon-loading/`toggle`/`open`/`close`/viewport-aware `positionPopup`/light-dismiss (outside-click + Escape) wiring, plus a `disconnectedCallback` that removes the document click listener (subclasses previously each leaked one). Subclasses supply their own template (incl. `<style>`) and override `open()` to seed fields before calling `super.open()` |
