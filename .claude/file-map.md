...
| `seDebugOverlay.js` | `<se-debug-overlay>` | Dev-mode "visibility" inspector — off by default, toggled per instance via `Editor.setDebugOverlay(enabled)`. Polls `svgCanvas.getDebugSnapshot()` and renders Selection / Group context / Path editing tables, flagging any entry the snapshot marks `stale: true` (rendered but no longer backed by the model — e.g. a selection box left shown after deselect, a path-node grip orphaned from a previously-drawn/edited path, group-context sibling dimming not cleared on `leaveContext()`). Caught a real live bug this way — see `path-actions.js`'s `mouseDown` hide-all-on-new-path guard and `tests/unit/path-draw-orphaned-grips.test.js` |
