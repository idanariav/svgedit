...
| `tool_mirror` | Mirror drawing toggle | — | Injected into `#editor_panel` by ext-mirror. Click = vertical center axis, **Shift+click** = horizontal; click again to turn off. While on, a dashed accent axis is drawn (`#mirrorAxis` on `#canvasBackground`) and every freshly drawn element gets a reflected twin (`se:mirror-of`) in the **same undo step** (the extension wraps `addCommandToHistory`, batching source + twin). Text twins are position-mirrored only (no glyph flip). API: `svgCanvas.setMirrorAxis('v'\|'h'\|null)` / `getMirrorAxis()` |
...
| `.selected_panel` "Object" | single element | `tool_topath`, `tool_smooth_path` (paths only → `svgCanvas.smoothSelectedPath()`, paper.js flatten→simplify refit), `tool_path_offset` (`<se-offset-settings>` popover → `svgCanvas.offsetPath(delta)`), `tool_repeat` (`<se-repeat-settings>` popover → `svgCanvas.repeatSelection(params)`, radial/grid array with `se:repeat*` re-edit stamps — see ext-repeat), `tool_mirror_copy` (→ `svgCanvas.mirrorSelection()`, reflected copy across the canvas axis), `tool_stroke_to_path` (→ `svgCanvas.strokeToPath()`), `tool_reorient`, `tool_make_link`; nested `.container_panel` (`g_title`), `.use_panel` (`tool_unlink_use`), `.a_panel` (`link_url`) |
| `.multiselected_panel` "Combine" | 2+ elements | `tool_bool_union/intersect/subtract/exclude/divide`, `tool_clip_set`, `tool_mask_set`, `tool_make_link_multi` (boolean ops & clip/mask require exactly 2; `divide` produces two separate path pieces), `tool_repeat_multi` + `tool_mirror_copy_multi` (repeat/mirror-copy the whole selection as a unit; injected by ext-repeat / ext-mirror) |
