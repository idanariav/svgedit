...
| `tool_mirror` | Mirror drawing toggle | — | Injected into `#editor_panel` by ext-mirror. Click = vertical center axis, **Shift+click** = horizontal; click again to turn off. While on, a dashed accent axis is drawn (`#mirrorAxis` on `#canvasBackground`) and every freshly drawn element gets a reflected twin (`se:mirror-of` + `se:mirror-axis`) in the **same undo step** (the extension wraps `addCommandToHistory`, batching source + twin). Twins stay **live-linked**: editing/moving the source re-syncs the twin (even mid-drag, mode on or off); dragging the twin itself unlinks it. Text twins are position-mirrored only (no glyph flip). API: `svgCanvas.setMirrorAxis('v'\|'h'\|null)` / `getMirrorAxis()` |
...
- `tool_mirror` toggle in the view tray: click = vertical canvas-center axis, Shift+click = horizontal, click again = off. Dashed accent axis overlay (`#mirrorAxis` on `#canvasBackground`, redrawn on zoom/resolution change)
...
- Twin = clone + `translate(w 0) scale(-1 1)` (or the horizontal analog) prepended, `se:mirror-of="<sourceId>"` stamped; `<text>` is position-mirrored only (glyphs stay readable). Twins are independent after creation (live linked symmetry = Wave 2)
- **Mirror-copy selection** (`tool_mirror_copy` / `tool_mirror_copy_multi`, Object/Combine sections): `svgCanvas.mirrorSelection()` reflects a copy of the selection across the canvas axis (vertical when the mode is off) as one batch. API: `setMirrorAxis('v'|'h'|null)`, `getMirrorAxis()`
