...
| `tool_wireframe` | Wireframe mode | F | In `#editor_panel` (view tray). `clickWireframe()` toggles the `.wireframe` class + `#wireframe_rules` style, then (a) sets `svgCanvas.getCurConfig().wireframeSnapping` and (b) calls `svgEditor.updateProportionMarkers()`. While on: ext-proportion-markers draws edge proportion ticks, and dragging snaps the selection's edges/center to canvas proportion lines (snap logic in `core/event.js`, select-mode move) |
| `tool_canvas_settings` *(`<se-canvas-settings>`)* | Canvas resize popover | — | In `#editor_panel`. Opens a popover with W/H spin-inputs, size presets (2-col grid), and Apply/Reset. Each preset shows ratio + size — aspect presets `4:5 (800:1000)` / `5:4 (1000:800)` / `16:9 (1000:563)` / `1:1 (1000:1000)` (base 1000px) plus the predefined `4:3` sizes moved out of Document Properties (`640:480`…`1600:1200`). The preset list is **user-editable**: a "Manage presets" toggle flips the grid into editable rows (label + W + H + delete ×, "+ Add preset", Save/Cancel); the label auto-fills from the W:H GCD until the user overrides it. The curated list persists through the host `userDataAdapter` (`getCanvasPresets`/`setCanvasPresets`) — in Obsidian into `data.json` — else `localStorage` key `svg-edit-canvas-presets`; the built-in `DEFAULT_PRESETS` seed an empty/invalid list. Talks to `svgCanvas.setResolution`/`getResolution` directly via the global `svgEditor`. The popover also has a **Layouts** section (saved canvas templates): "+ Save current" stores the whole canvas (size, `bkgd_color`, full SVG) under a name; each saved row offers **Apply** / **Overwrite** (⤓) / **Remove** (×). Applying overwrites only size + background and re-injects the saved objects (fresh remapped ids) onto a `Layout: <name>` layer left inactive (placeholders locked by `pointer-events:none` until the user selects that layer); pre-existing canvas objects are kept. Logic lives in `src/editor/canvasLayouts.js`, persisted via `getCanvasLayouts`/`setCanvasLayouts` else `localStorage` key `svg-edit-canvas-layouts` |
| `grid_settings` *(`<se-grid-settings>`)* | Grid settings popover | — | Injected into `#editor_panel` by ext-grid. Show/snap toggles, shape select (square/isometric/triangle/1pt/2pt perspective/**thirds/golden ratio/center cross**), grid color, snapping step. The last three shapes are composition-guide crosses (fixed fractional lines, not a repeating pattern) — folded into the grid shape list so the old separate guides/compositions popover could be removed entirely |
| `tool_smart_snap` | Smart snapping toggle | — | Injected into `#editor_panel` by ext-smart-guides. Toggles object-to-object alignment snapping while dragging (edge/center lines + equal-spacing hints, incl. page edges/center). Default **on**; persisted as the `smart_snapping` pref; config flag `smartSnapping`. Snap math in `core/smart-guides.js`, applied in the `event.js` select-move branch; guide overlay `#smartGuides` (appended last in `svgroot`, above content). Same-kind matches (edge↔edge/center↔center) beat equal-spacing beat mixed (edge↔center, reduced tolerance) |
...
**Mirror drawing toggle removed from the top panel.** `tool_mirror` (the
view-tray button that turned the live-symmetry drawing mode on/off) no longer
has a toolbar entry point. The mode itself — `svgCanvas.setMirrorAxis('v'\|'h'\|null)`
/ `getMirrorAxis()`, live-linked twin sync, mirror-delete — is unchanged in
`ext-mirror.js`, just API-only for now. The unrelated "Mirror-copy selection"
buttons (`tool_mirror_copy`/`_multi`, one-shot reflected copy) stay in the
Design tab's Object/Combine sections — see below.
...
| `palette` | Color palette | Quick color swatches. A **target button** (right of the pencil) cycles the destination a clicked swatch paints — fill → stroke → fill — its icon reflecting the active target. Background isn't in the cycle; it has its own dedicated picker (`bg_color`). The pencil toggles edit mode. In edit mode a swatch opens the full color dialog, so an override can be a **solid color or a linear/radial gradient** (gradients are stored as `{type, xml, alpha}` and render as a CSS-gradient swatch; clicking dispatches a ready-made `Paint`). A global reset appears while editing if any overrides exist |
...
| Stroke & Opacity *(always shown in tab)* | always | `stroke_width` (0–99, label "Stroke Width"), `opacity` (0–100%), `stroke_style` (dash), `stroke_linejoin` / `stroke_linecap` — each in its own labeled column ("Join" / "Cap") within a shared `.sidepanel_section_grid`, no more combined "Joins & caps" row. **Moved from bottom panel**; `change` listeners bound in `TopPanel.init` (delegate to `bottomPanel` handlers) |
...
| `#corner_panel` "Corners" | single path/polygon/polyline with straight segments only (injected by ext-corner-radius, before the Object section) | `corner_radius_value` (0–500) → `svgCanvas.applyCornerRadius(r)`. Attribute-driven fillet: pre-fillet geometry kept in `se:orig-d`, radius in `se:corner-radius`; radius 0 fully restores; geometry in `core/corner-radius.js`; `coords.js` `remapElement` keeps `se:orig-d` in sync with baked moves/scales. Node-editing a rounded path drops the (stale) rounding attrs |
| `.selected_panel` "Object" | single element | Buttons are split into two labeled `.sidepanel_btn_group` clusters (hairline divider between them) instead of one flat row. **"Path"** cluster: `tool_topath` (hidden for `image`/`text`/`path`/`g`/`use` — nothing to convert), `tool_smooth_path` (freehand-drawn paths only — requires `data-freehand="1"`, stamped onto the `<path>` by `simplifyFreehand`/`smoothPolylineIntoPath` when a pencil stroke commits (`core/path-simplify.js`, `core/path-actions.js`'s `#smoothPolylineIntoPath`); the curve-fit algorithm assumes a dense freehand point cloud and distorts a node-tool-authored or Convert-to-Path path's deliberate geometry, so those stay excluded even though `elname === 'path'`. `<se-smooth-path-settings>` popover — 0-100% strength spin-input, live-previews via `svgCanvas.previewSmoothPath(strength)` on every change with no undo step, `Apply` commits one undo step via `svgCanvas.commitSmoothPath()`, closing without applying reverts via `svgCanvas.cancelSmoothPath()`; strength always re-fits from the `d` captured when the popover opened, so repeated adjustments in one session never compound — see `core/path-simplify.js`), `tool_path_offset` (non-arc paths only; `<se-offset-settings>` popover → `svgCanvas.offsetPath(delta)`), `tool_stroke_to_path` (hidden for `path` elements outright — by design, use node editing on a path directly — and otherwise hidden when the element has no visible stroke: `stroke="none"`, `stroke-width` not `> 0`, or `stroke-opacity`/`opacity` of `0` (`hasVisibleStroke()` in `core/path-offset.js`, missing `stroke-width` defaults to the SVG initial value of 1 since `cleanupElement()` strips it at that value); → `svgCanvas.strokeToPath()`) — plus extension buttons dynamically anchored after `tool_path_offset`/`tool_stroke_to_path` so they land in this same cluster: `tool_repeat` (`<se-repeat-settings>` popover → `svgCanvas.repeatSelection(params)`, radial/grid array with `se:repeat*` re-edit stamps, radial center = canvas/own/custom-picked point — see ext-repeat), `tool_segment` (`<se-segment-settings>` popover → `svgCanvas.segmentSelection(params)`, radial/grid dividing lines around the shape's own center, with a Split/Non-split toggle, `se:segment` re-edit stamp on the non-split `<g>` result — see ext-segment), `tool_mirror_copy` (→ `svgCanvas.mirrorSelection()`, reflected copy across the canvas axis), `tool_taper` (`<se-taper-settings>` popover → `svgCanvas.applyTaperStroke({start,end})`; shown only when `svgCanvas.canTaperStroke` — see ext-taper). **"Select & Link"** cluster: `tool_select_same` (se-list: Same fill / Same stroke / Same type → `svgCanvas.selectSameAs(criterion)`; scans visible layers, or the current group context), `tool_make_link`. Any new extension button anchored via `$id('tool_path_offset')`/`$id('tool_stroke_to_path')` stays in the "Path" cluster automatically; nested `.container_panel` (`g_title`), `.use_panel` (`tool_unlink_use`), `.a_panel` (`link_url`) sit below both clusters, unchanged |
| `.multiselected_panel` "Combine" | 2+ elements | **Boolean** subsection: `tool_bool_ops`, an `se-list` dropdown (fixed `bool_ops.svg` trigger face, no `track-selection` — a one-shot action menu like `tool_arrange`, not a persistent setting) holding `se-list-item`s that carry **both** an icon (`src="bool_union.svg"` etc — the original per-action icons) **and** a short visible text label (`option="tools.bool_*_label"` — "Union"/"Intersect"/"Subtract"/"Exclude"/"Divide"), plus the original long `title="tools.bool_*"` description as hover tooltip. Icon+label rows required a `seListItem.js` change: it used to set `option` by overwriting `$menuitem.textContent` (which silently wiped the icon), so a dedicated `.label-text` span was added and `option` now writes there instead, leaving `.icon-wrap` intact — see the icon+label CSS rule keyed off `:host([option][src])`. Replaced five separate small `se-button`s (hard to visually distinguish, previously required hovering each icon for its tooltip) — its `change` event is handled by `TopPanel.clickBoolOps`, which switches on `evt.detail.value` (`union`/`intersect`/`subtract`/`exclude`/`divide`) to the pre-existing `clickBoolUnion`/`clickBoolIntersect`/`clickBoolSubtract`/`clickBoolExclude`/`clickBoolDivide` methods (unchanged, still call `svgCanvas.boolean*()`). Below it, a `sidepanel_btn_row` keeps: `tool_clip_set`, `tool_mask_set`, `tool_match_strokes` (→ `svgCanvas.matchStrokes()`: every stroked element in the selection — group members included — gets the primary element's stroke-width plus round joins/caps, one undo step), `tool_make_link_multi`. Boolean ops & clip/mask require exactly 2 selected — either side may be a `<g>`/`<a>` group, whose recursive leaf shapes are merged into one compound path via `core/boolean-ops.js`'s `collectLeaves`/`getAncestorMatrix`, honoring nested + group-own transforms; `divide` produces two separate path pieces. Since the five boolean-op ids moved from auto-registering `se-button`s to non-registering `se-list-item`s, their Hotkey Manager/Command Search/Favorites visibility is now provided by five keyless `Editor.shortcuts` entries (`bool_union`/`bool_intersect`/`bool_subtract`/`bool_exclude`/`bool_divide`, group `'Boolean'`) that call `topPanel.clickBoolUnion()` etc. directly — same pattern as the `move_to_front`/`align_left` keyless entries for the Arrange/Align dropdowns. `tool_shape_builder` (→ enters the interactive **shape builder** mode: drag across highlighted regions to merge, Alt to delete — see ext-shape-builder), `tool_text_on_path` (shown when exactly one text + one shape are selected → `svgCanvas.attachTextToPath()` — see ext-text-path), `tool_repeat_multi` + `tool_mirror_copy_multi` (repeat/mirror-copy for the whole selection as a unit; injected by ext-repeat / ext-mirror) all anchor themselves via `$id('tool_clip_set')?.parentElement` (not `tool_bool_union`, which no longer sits in that button row). The repeat popover now has a third **Path** mode (copies/start%/span%/rotate-along) that distributes the unit along the topmost selected path rail |

### Text tab (`#tab_text`)

| Section | Shown when | Controls |
|---------|-----------|----------|
| `.text_panel` "Text Style" | `<text>` (or all-text multi) | A manual "Font Family" `sub_label` sits above the font row (the `<se-font-select>` component has no built-in label attribute); `tool_font_family` (`<se-font-select>`, Google-style search + per-font previews; ext-fonts appends downloaded fonts), `tool_font_library` (`<se-font-library>`, Google Fonts browser + embed-on-export), `font_size` (1–1000), `tool_text_anchor` — **row alignment** left/center/right (the `se-list` keeps SVG `text-anchor` values `start`/`middle`/`end` but is **labelled/iconed** left/center/right via `align_left.svg`/`align_center.svg`/`align_right.svg`; it aligns multiline rows because every row's `<tspan>` shares the text's `x`), `tool_bold` (B), `tool_italic` (I), `tool_text_decoration_underline/linethrough/overline` |
...
| `#textpath_panel` "Text path" | `<text>` with a `textPath` child (injected by ext-text-path, self-managed visibility) | `textpath_offset` (startOffset 0–100% → `svgCanvas.textPathOffset(pct)`), `tool_text_path_release` (→ `svgCanvas.detachTextFromPath()`) |
...
| *(layer list)* | Layer rows with visibility toggle (`td.layervis`, eye icon) + **lock toggle** (`td.layerlock`, open/closed padlock) + **preview thumbnail** (`td.layerpreview`) + name |
...
| *(preview thumbnail)* | `td.layerpreview` holds a small inline `<svg class="layerpreview-thumb">` whose only child is a `<use href="#…">` pointing at the layer's own group element (`drawing.getLayerByName(name)`), viewBoxed to the full canvas (`svgCanvas.getResolution()`). Since the canvas isn't an iframe, the `<use>` lives in the same document and mirrors the group continuously — it reflects new/moved/recolored shapes in real time with **no manual redraw**, not just on the layer-list-changing events that call `populateLayers`. `populateLayers` lazily stamps an `id` on the group the first time it's previewed (via `svgCanvas.getNextId()`) if it doesn't already have one, since `<use>` can only target an id; the id persists for the group's lifetime and is otherwise identical in spirit to every other canvas element already carrying a `svg_N` id |
...
- **User shapes** (`extensions/ext-shapes/userShapes.js`, `localStorage` key `svg-edit-user-shapes`): "Add to Shape Library" (`EditorStartup._showAddToLibraryDialog`) saves the selection as `{ svgContent, bbox, linkedFile? }`. Before serializing, `EditorStartup._stripBackdropRects` drops a **full-canvas backdrop** from the selection — a *filled* `<rect>` whose stroked bbox covers the whole selection bounds (e.g. a page-background colour rect baked into the drawing) — so saved shapes don't bundle the page background; it never empties the selection and ignores outline-only (`fill:none`) frames. When a host exposes `window.svgEditHost.pickVaultFile`, the dialog shows an optional **Linked vault file** control; the chosen link is persisted as `linkedFile` and threaded through the `shape-insert` event detail. On insert, `ext-shapes.js` stamps `data-vault-link` onto the imported root **and every descendant** so the link survives ungroup / partial deletion (see [architecture.md](architecture.md) "Host bridge").
- **Library management** (modal only, in-shadow ⋮ menus reusing `_openDropdown`/`_fillDropdown`; outside-click close uses `composedPath()` so submenu clicks aren't mis-read as outside the shadow boundary):
  - **Shape ⋮ menu** (`.sl-shape-menu`, user shapes only — built-in path-string shapes stay read-only): **Rename** (inline-edit via `_startInlineRename` → `renameUserShape`), **Move to…** (keepOpen submenu of all other categories → `moveUserShape`), **Remove from library** (`removeUserShape`).
  - **Category ⋮ menu** (`.sl-cat-menu`, every sidebar category except the virtual "All"): **Rename** + **Delete** (user categories, `confirm()` → `deleteUserCategory`, removes its shapes) / **Hide** (built-in categories, `hideCategory` — non-destructive). User-category rename uses `renameUserCategory`, which operates on the real category key and **merges** into an existing category when names collide; built-in rename uses `setCategoryLabel` (display-label override only).
  - **Hidden built-in categories** are non-destructive: schema fields `categoryLabels: { [catId]: string }` (display-name overrides honoured in `_catLabel`) and `hidden: string[]` (built-in ids filtered out in `_rebuildCategoryList`). A collapsible **"Hidden (N)"** section under the sidebar (`_buildHiddenSection`) lists hidden categories with a **Restore** button (`unhideCategory`). Bundled `shapelib/*.json` is never mutated.
...
- Snapping is shape-aware (`snapPointToGrid` in `dom-utils.js`): square/perspective snap per-axis; isometric/triangle snap to lattice nodes. The line-grid spacing is `step·sin(60°)` so visible intersections coincide with snap nodes.
...
- **Cutter** (`tool_cutter`): affects selected shapes only (or all shapes if nothing selected). Two ways to cut:
  - **Drag** (press-move-release in one motion) → instant straight-line cut, same as before.
  - **Click** (press-release with no real movement) → starts a multi-point zigzag line instead: click to add each further vertex, **Enter** or **double-click** to cut along the resulting polyline, **Backspace/Delete** removes the last vertex, **Escape** cancels. Useful for non-straight cuts (e.g. a jagged "broken egg" crack through an ellipse).
- **Shift** (while dragging, or when placing/previewing a vertex) snaps that segment to the nearest 15° angle from its start point (same `snapToAngle` helper from `core/math.js` used by the path tool)
- Core geometry: `core/cutter.js`'s `cutShapes(points)` — 2 points (straight) and 3+ points (zigzag) both go through the same exact boundary-splice method (`getIntersections`/`divideAt`) that handles any even number `2m` of boundary crossings per loop (a Weiler-Atherton-style stack decomposition), producing `m + 1` pieces — e.g. an "S"-shaped cut that dips into and out of a shape twice yields 3 pieces. A target with multiple disjoint closed loops in one `d` (a compound path) is cut loop-by-loop — a loop the line/polyline doesn't cross is left completely untouched, not swept up wholesale

### ext-curvature — Curvature Tool (`extensions/ext-curvature/`)
- **Curvature** (`tool_curvature`): Click to place anchor points; smooth curves are auto-computed via **Spiro** (clothoid/curvature-continuous, `spiro` npm package) — the tool's only smoothing mode, no mode selector. Shift+click for a sharp corner anchor. **Alt+click** places an **end** anchor — the curve up to that point is fixed (points added afterward can never reshape it), and the end anchor also acts as the **start** of the next segment; "start"/"mid" are purely positional/derived, not stored — only `corner`/`end` are per-point booleans, and they're independently combinable (**Alt+Shift+click** = an end anchor that's also a sharp corner). Alt (not Ctrl) was chosen deliberately: Ctrl+click maps to a secondary-click/context-menu on macOS and some Linux setups. Click-dragging an already-placed anchor (any of start/mid/end) repositions it and live-reshapes its adjoining segment(s) for the remainder of the drawing session; this is session-only — no anchor metadata is persisted once the path is finalized. Double-click (or click near start) finalizes/closes the path. Switching to any other tool mid-session also finalizes the path open (a `modeChange` listener calls the same `finalize(false)` used by double-click) rather than leaving the dashed preview/anchor dots behind as real layer content. `buildPathD` in `ext-curvature.js` splits points into independent segments at each `end` anchor (`splitIntoSegments`) before dispatching each to `buildSpiro`/`buildCatmullRom` — with zero `end` anchors it degrades to the original single global solve, so plain click-only drawing is unchanged. Note: the file's own `keyDown` hook (intended to finalize open via Escape) is currently dead code — `runExtensions('keyDown', …)` is never called anywhere in `event.js`/`Editor.js`/`Hotkeys.js`, a pre-existing gap unrelated to the anchor feature.

### ext-puppet-warp — Puppet Warp Tool (`extensions/ext-puppet-warp/`)
- **Puppet Warp** (`tool_puppet_warp`, mode `puppetwarp`, position 13): Illustrator-style mesh deformation. Select an object (or a **group** of shapes) and enter the tool; the selection's geometry at entry is the session's **rest pose**. Click empty canvas to drop a **pin** (a fixed anchor); **drag** an existing pin to bend the shape smoothly around the pins left fixed. Switching tools **commits** the pose as one undo step; **Escape cancels** (restores rest geometry, no undo entry). Only `path`/`rect`/`circle`/`ellipse`/`line`/`polyline`/`polygon` are warpable (`WARPABLE` set); `text`, `image`, and `use` inside a warped group are left stationary — defensible v1 scope, surfaced in the toolbar tooltip.
- **Deformation math** lives in `mls.js` (pure, unit-tested in `tests/unit/ext-puppet-warp-mls.test.js`): **Moving Least Squares rigid** deformation (Schaefer et al., SIGGRAPH 2006) — each point gets a per-point rigid (rotation+translation, no shear) transform weighted by inverse distance to the pins, so limbs bend without shearing. `deformPoint(v, pins, alpha)` reproduces a global rigid transform exactly.
- **Geometry pipeline** (`ext-puppet-warp.js`): on entry, primitives are `convertToPath`'d, selection grips are cleared (so they don't eat drags), and each target path is resampled via paper.js into dense **content-space** polylines — sampled at uniform **arc-length** steps (~6 units), which is what lets *straight* limbs bend (paper's own `flatten` only splits curved spans and would leave a straight line as 2 unbendable points). Element-local `d` coords are mapped to content space with `matrixMultiply(getMatrixToContent(el), getMatrix(el))` — the element's **own** transform (`getMatrix`) *and* its `<g>`/`<a>` ancestors' (`getMatrixToContent`); using `getMatrixToContent` alone silently drops a path's own `transform` and warps in the wrong frame. Each drag re-warps from the cached rest and rewrites `d` live as a dense polyline (fast); on **commit** that polyline is refit to smooth cubic béziers via `svgCanvas.simplifyPathD(d, tol)` (the reusable helper exposed from `core/path-simplify.js`, tol=2), so the baked path is compact and doesn't grow across re-pose sessions. Overlay pins are `<circle>`s in the current layer (`pointer-events:none`, filled with `var(--accent)` for the dragged pin), removed on exit.
- **Atomic, cancelable undo**: `convertToPath` auto-commits its own undo entry, so the session captures each conversion command off the undo stack (`undoMgr.undoStack`/`undoStackPointer`) and folds it into the single **Puppet Warp** BatchCommand on commit (conversions first, then per-target `d` change) — one undo step reverses warp→convert. Escape (or exiting without dragging) `unapply`s those captured conversions so the primitives return and no stray undo entry is left.
- **Model is session-only** ("re-pose from current shape each time") — no persistent rig / `se:` metadata; the baked shape becomes the next session's rest.
- **Escape** is handled by a real `window` keydown listener guarded on mode, **not** the extension `keyDown` hook (which is dead code in this build — `runExtensions('keyDown', …)` is never dispatched), mirroring ext-curvature's `window` dblclick workaround.
...
- Implements `svgCanvas.showSmartGuides(payload)`: solid accent alignment lines spanning the moving box + matched target, dashed equal-spacing gap segments with end ticks. Overlay `<svg id="smartGuides">` appended **last** in `svgroot` (above content, unlike `#canvasBackground` overlays), position synced from `#svgcontent` per draw, cleared on mouse-up by core
- `tool_smart_snap` toggle in the view tray; `smartSnapping` config flag, `smart_snapping` pref (default on)
- Skipped while a group context is active (child bboxes aren't content-space); page edges/center are a synthetic target so canvas-centering snaps on an empty page

### ext-corner-radius — Corner Rounding (`extensions/ext-corner-radius/`)
- Injects the **Corners** section (`#corner_panel`, `corner_radius_value` spin input) into `#tab_design` before the Object section; shown for a single straight-segment `path`/`polygon`/`polyline` (never in pathedit)
- Calls `svgCanvas.applyCornerRadius(r)` (`core/corner-radius.js`): polygon/polyline is swapped for a `<path>` in the same batch; every straight-corner vertex becomes an `A` arc fillet with per-corner trim clamping (`min(r/tan(θ/2), len/2, len/2)`); one `BatchCommand` per edit
- Data model: `se:corner-radius` + `se:orig-d` (canonical absolute M/L/Z source; `se:` attrs bypass sanitize so they survive save/load). Radius 0 restores the source and removes both. `coords.js` `remapElement` calls `remapCornerSource` so baked moves/scales/flips keep the source + radius in sync (regenerates `d` from source — no stale-geometry teleport)
- If `d` is rewritten outside the pipeline (pathedit node edits etc.), the extension detects the mismatch on selection and silently drops the rounding attrs (shape untouched)

### ext-repeat — Radial / Grid Repeat (`extensions/ext-repeat/`)
- `<se-repeat-settings>` popover buttons: `tool_repeat` (Object section, after the offset popover) + `tool_repeat_multi` (Combine section). Popover: Radial (copies 1–36, sweep°, center = canvas/own-selection/**custom**) or Grid (rows, cols, gap X/Y)
- `svgCanvas.repeatSelection(params)`: clones the selection once per step — radial prepends `rotate(angle cx cy)` (never recalculated: baking would decompose the rotates), grid prepends `translate(j·stepX, i·stepY)` with step = stroked bbox + gap. Clones get fresh ids via `remapElementIdsAndRefs` (defs/paint servers stay shared). One `BatchCommand('Repeat')`
- **Custom center**: `center: 'custom'` + `centerX`/`centerY` in params, set by clicking a point on the canvas (never typed) via the popover's "Pick center" button → `svgCanvas.armRepeatCenterPick(cb)` arms a one-shot mode `'repeat-pick-center'` (added to `EditorStartup.js`'s `modesToCancel` for Escape, and its crosshair-cursor switch); the extension's own `mouseDown` hook (guarded on that mode) captures the next canvas click, returns to select, and invokes the callback. The source element is **never moved** — whatever distance it already sits from the chosen center becomes every copy's radius, exactly like the existing canvas/own-center behavior, just with an arbitrary pickable center (this is the flower-petal use case: draw one petal off-center, pick the flower's center, fan copies around it)
- Re-editable: source stamped `se:repeat-source="<uid>"` + params as `se:repeat`; copies get `se:repeat-copy="<uid>"`. Re-opening the popover (source **or any copy** selected) pre-fills via `svgCanvas.getRepeatParams()`; Apply→Update deletes copies by uid and regenerates in the same batch. Copies are plain elements — files render identically anywhere
- `[data-frame]` rects are never repeated

### ext-segment — Segment / Shape Divide (`extensions/ext-segment/`)
- `<se-segment-settings>` popover button: `tool_segment` (Object section, after `tool_repeat`). Popover: Radial (lines 2–72, start angle°) or Grid (lines 1–72, axis = vertical/horizontal), plus a Split/Non-split toggle. No center picker — radial mode always divides around the shape's own center (`core/segment.js`'s `resolveCenter`: exact `cx`/`cy` for an untransformed circle/ellipse, bbox center otherwise)
- `svgCanvas.segmentSelection(params)` (`core/segment.js`): **split** cuts the shape into N (radial) or N+1 (grid) independent `<path>` pieces — each piece is `shapePath.intersect(wedgeOrStripPolygon)` (paper.js), one generated polygon per resulting piece rather than one N-way multi-line divide (more robust — see the module's own doc comment); **non-split** draws the N dividing lines on top, exactly clipped to the shape's true outline via `getIntersections()` (not an approximation to its bounding box), and wraps shape+lines in a new `<g>` so they act as one object. Either way, one `BatchCommand`
- Re-editable (non-split only — split pieces are independent, nothing to re-edit): the wrapper `<g>` is stamped `se:segment="radial;count=…;split=0;…"`; `svgCanvas.getSegmentParams()` seeds the popover ("Update") whether the `<g>` or the wrapped shape itself is selected. Switching an existing non-split result to Split first removes the old lines and unwraps the shape back out of the (then-empty) group before cutting, so no stale group is left behind
- Visibility gated on `svgCanvas.canSegment(elem)` — any single element except `text`/`tspan`/`image`/`use`/`symbol`/`g`/`defs`; v1 does not support multi-selection or arbitrary group input

### ext-mirror — Mirror Drawing (`extensions/ext-mirror/`)
- Mirror drawing mode is **API-only** — the `tool_mirror` view-tray toggle button was removed; the mode (click = vertical canvas-center axis, Shift+click = horizontal, in the old UI) is driven entirely through `svgCanvas.setMirrorAxis('v'|'h'|null)` / `getMirrorAxis()` now. Dashed accent axis overlay (`#mirrorAxis` on `#canvasBackground`, redrawn on zoom/resolution change) still renders whenever the axis is set
- While on, each freshly drawn element gets a reflected twin: the extension **wraps `svgCanvas.addCommandToHistory`** and, for a bare `InsertElementCommand` of a layer child (the hand-drawn commit path), replaces it with one `BatchCommand('Draw mirrored')` holding source + twin — so undo removes both with one Ctrl+Z. Batched inserts (paste, repeat, boolean ops) pass through untouched; `[data-frame]`, group-context drawing, and its own twins are skipped
- Twin = clone + `translate(w 0) scale(-1 1)` (or the horizontal analog) prepended, `se:mirror-of="<sourceId>"` stamped; `<text>` is position-mirrored only (glyphs stay readable). Twins stay live-linked to their source (see extensions.md)
- **Mirror-copy selection** (`tool_mirror_copy` / `tool_mirror_copy_multi`, Object/Combine sections): `svgCanvas.mirrorSelection()` reflects a copy of the selection across the canvas axis (vertical when the mode is off) as one batch — unaffected by the toggle button's removal
...
- Controls: **Hue** (`color_shift_h`, −180 to 180°), **Saturation** (`color_shift_s`, ±100), **Lightness** (`color_shift_l`, ±100), **Transparency** (`color_shift_t`, ±100 — positive = more transparent), **Fill** / **Stroke** checkboxes (`color_shift_fill` / `color_shift_stroke`, both on by default), **Reset** button (`color_shift_reset`). Field labels were previously bare `H`/`S`/`L`/`T`; now full words
...
- Controls: **Angle** (`shadow_angle`, 0–359°, clockwise from 12 o'clock), **Distance** (`shadow_length`, 0–500 px stretch — labelled "Distance", id/variable still `length` internally), **Blur** (`shadow_blur`), **Opacity** (`shadow_opacity`), **Color** (`shadow_color`, with a "Shadow color" caption next to the swatch), **Remove** button (`shadow_remove`). Field labels were previously bare `∠`/`L`/`B`/`%`; now full words
...
| U | Edit SVG source |
| D | Clone element |
| Ctrl/Cmd+D | Transform again — `svgCanvas.transformAgain()`: clone the selection offset by the last committed move delta (drag or nudge, tracked in `svgCanvas.lastMoveDelta`); repeat for evenly spaced chains |
