...
| `tool_smart_snap` | Smart snapping toggle | — | Injected into `#editor_panel` by ext-smart-guides. Toggles object-to-object alignment snapping while dragging (edge/center lines + equal-spacing hints, incl. page edges/center and **ruler guides** from ext-guides — single-axis `isGuide` targets, highlighted full-length on match). Default **on**; persisted as the `smart_snapping` pref; config flag `smartSnapping`. Snap math in `core/smart-guides.js`, applied in the `event.js` select-move branch; guide overlay `#smartGuides` (appended last in `svgroot`, above content). Same-kind matches (edge↔edge/center↔center) beat equal-spacing beat mixed (edge↔center, reduced tolerance) |
...
| `tool_guides` *(`<se-guides-settings>`)* | Guides & composition overlays popover | — | Injected into `#editor_panel` by ext-guides. Popover: **Thirds / Golden / Center** overlay toggles (drawn in `#compOverlays` on `#canvasBackground`; `guides_*` prefs), **+ Vertical / + Horizontal** guide drops, and **Clear all guides**. Guides render in the interactive `#rulerGuides` overlay (above content): drag to reposition, drag off the canvas to delete; with rulers enabled (Editor Options — this fork defaults them off) drag new guides straight off a ruler. Guides persist in the document as `se:guides` on `#svgcontent` and act as snap targets while dragging objects (via `core/smart-guides.js`) |
...
- Core geometry: `core/cutter.js`'s `cutShapes(points)` — 2 points use the half-plane intersection method; 3+ points use an exact boundary-splice method (`getIntersections`/`divideAt`) that handles any even number `2m` of boundary crossings (a Weiler-Atherton-style stack decomposition), producing `m + 1` pieces — e.g. an "S"-shaped cut that dips into and out of a shape twice yields 3 pieces
...
- **Puppet Warp** (`tool_puppet_warp`, mode `puppetwarp`, position 13): Illustrator-style mesh deformation. Select an object (or a **group** of shapes) and enter the tool; the selection's geometry at entry is the session's **rest pose**. Click empty canvas to drop a **pin** (a fixed anchor); **drag** an existing pin to bend the shape smoothly around the pins left fixed. Switching tools **commits** the pose as one undo step; **Escape cancels** (restores rest geometry, no undo entry).
