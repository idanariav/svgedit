...
| `.selected_panel` "Object" | single element | `tool_topath`, `tool_smooth_path` (paths only → `svgCanvas.smoothSelectedPath()`, paper.js flatten→simplify refit), `tool_path_offset` (`<se-offset-settings>` popover → `svgCanvas.offsetPath(delta)`), `tool_repeat` (`<se-repeat-settings>` popover → `svgCanvas.repeatSelection(params)`, radial/grid array with `se:repeat*` re-edit stamps, radial center = canvas/own/custom-picked point — see ext-repeat), `tool_mirror_copy` (→ `svgCanvas.mirrorSelection()`, reflected copy across the canvas axis), `tool_motion_lines` (`<se-motion-settings>` popover → `svgCanvas.applyMotionLines(params)`, speed-line generator with `se:motion*` re-edit stamps — see ext-motion-lines), `tool_stroke_to_path` (→ `svgCanvas.strokeToPath()`), `tool_taper` (`<se-taper-settings>` popover → `svgCanvas.applyTaperStroke({start,end})`; shown only when `svgCanvas.canTaperStroke` — see ext-taper), `tool_reorient`, `tool_select_same` (se-list: Same fill / Same stroke / Same type → `svgCanvas.selectSameAs(criterion)`; scans visible layers, or the current group context), `tool_make_link`; nested `.container_panel` (`g_title`), `.use_panel` (`tool_unlink_use`), `.a_panel` (`link_url`) |
